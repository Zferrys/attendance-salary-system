...
<!DOCTYPE html>
<html lang="zh-CN">
<head><meta charset="UTF-8"><meta name="_csrf" content="${pageContext.session.getAttribute('CSRF_TOKEN')}"><title>薪资详情 - 考勤薪资系统</title>
<link rel="stylesheet" href="${pageContext.request.contextPath}/assets/css/style.css">
<script src="${pageContext.request.contextPath}/assets/js/common.js"></script>
<style>
    .salary-card { max-width: 600px; margin: 0 auto; }
    .salary-header { text-align: center; padding: 24px; background: linear-gradient(135deg, #3b82f6, #06b6d4); color: #fff; border-radius: 10px 10px 0 0; }
    .salary-header h2 { font-size: 20px; margin-bottom: 4px; }
    .salary-header p { opacity: 0.75; font-size: 13px; }
    .salary-body { padding: 24px; background: var(--surface); }
    .salary-row { display: flex; justify-content: space-between; padding: 12px 0; border-bottom: 1px dashed var(--border); font-size: 14px; }
    .salary-row .label { color: var(--ink-secondary); }
    .salary-row .value { font-weight: 600; color: var(--ink); }
    .salary-row.total { border-bottom: none; border-top: 2px solid var(--brand); margin-top: 8px; padding-top: 16px; }
    .salary-row.total .value { font-size: 20px; color: var(--success); }
    .salary-footer { text-align: center; padding: 16px; background: var(--surface-hover); border-radius: 0 0 10px 10px; font-size: 12px; color: var(--ink-muted); }
    .history-list { margin-top: 20px; }
    .history-item { display: flex; justify-content: space-between; align-items: center; padding: 12px 16px; background: var(--surface-hover); border-radius: 8px; margin-bottom: 8px; font-size: 14px; transition: all 0.2s; }
    .history-item:hover { background: var(--brand-soft); }
    .history-item .month { font-weight: 600; color: var(--ink); }
    .history-item .amount { font-weight: 700; color: var(--success); }
    @media print {
        .navbar, .breadcrumb, .filter-bar, .rule-box, .salary-footer button { display: none !important; }
        .main-container { padding: 0 !important; }
        .salary-card { box-shadow: none !important; border: 2px solid var(--brand) !important; page-break-inside: avoid; }
